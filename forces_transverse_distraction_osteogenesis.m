% Overall code for calculating Transverse Distraction Osteogenesis (TDO) distraction forces in the fibula:
clc
close all
clear all
l_0_l = 1; % initial gap width for callus formation
l_delta_l = 0.33; % distraction step size for purely lateral strain
DL = 10; % distraction length [mm]
S = round(DL/l_delta_l); % number of distraction steps
T_s = 8; % time step size in hours
k = 1.5; % scaling factor from Meyers
k_A = 1.5; % area scaling
T_end = S*T_s*60*60; % total duration of distraction in seconds (each step)
DT = 1; % time step size
...
A_ini = 0.002306; % distracted area in m^2
